...
def main() -> int:
    ...
    algorithm_changes = [
        item for item in changes if not item["path"].startswith("experiment.")
    ]
    print(json.dumps({"all_changes": changes, "algorithm_change_count": len(algorithm_changes)}, ensure_ascii=False, indent=2))
    return 0
